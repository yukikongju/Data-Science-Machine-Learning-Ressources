...
def sarsa(env, gamma=1.0, alpha_init = 0.5, alpha_min = 0.01, 
          alpha_decay_ratio=0.05, epsilon_init=0.3, epsilon_min=0.01, 
          epsilon_decay_ratio=0.05, n_episodes=1000, max_steps=200):
    # initialization
    nS, nA = env.observation_space.n, env.action_space.n
    Q = np.zeros((nS, nA), dtype=np.float64)
    Q_track = np.zeros((n_episodes, nS, nA), dtype=np.float64)
    alphas = decay_schedule(init_alpha, alpha_min, alpha_decay_ratio, n_steps)
