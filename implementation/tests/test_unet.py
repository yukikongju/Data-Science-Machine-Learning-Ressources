...
def example_up_tensor():
    ...
    return torch.randint(1, 1000, size=(1, 128, 64, 64)).float()
...
def test_unet_transpose(example_image): # TODO
    B, C, H, W = example_image.size()
    NUM_CLASSES = 10
    unet = UNet(n_channels=3, n_classes=NUM_CLASSES, bilinear=False)
    output = unet(example_image)
    assert output.size() == (B, NUM_CLASSES, H, W)

def test_unet_bilinear(example_image): # FIXME
    NUM_CLASSES = 10
    unet = UNet(n_channels=3, n_classes=NUM_CLASSES, bilinear=True)
    output = unet(example_image)
